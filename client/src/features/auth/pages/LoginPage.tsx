...
import { loginRequest, googleAuthRequest } from "../api/auth.api";
...
export function LoginPage(): JSX.Element {
    ...
    const onSubmit = async (values: LoginFormValues) => {
        try {
            ...
            const message = error.response?.data?.message || error.message || "Incorrect email or password";
            toast.error(message);
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-surface px-4 py-16">
            <div className="w-full max-w-md bg-white p-8 md:p-10 rounded-3xl border border-champagne-300/50 shadow-premium">
                <div className="text-center mb-8">
                    ...
                    <p className="font-body-md text-on-surface-variant text-sm">Welcome back to the gallery.</p>
                </div>
                ...
                <div className="relative flex py-2 items-center mb-6">
                    ...
                    <span className="flex-shrink mx-4 text-on-surface-variant font-caption text-xs uppercase tracking-widest">or sign in with email</span>
                    <div className="flex-grow border-t border-champagne-300/40"></div>
                </div>

                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div>
                        <label className="block font-label-md text-xs text-primary font-bold uppercase tracking-wider mb-2">Email Address</label>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <label className="font-label-md text-xs text-primary font-bold uppercase tracking-wider">Password</label>
                            <Link to="/forgot-password" className="font-caption text-xs text-champagne-700 hover:text-primary transition-colors font-semibold">Forgot Password?</Link>
                        </div>
                ...
                <p className="mt-8 text-center font-caption text-xs text-on-surface-variant">
                    New to Ashnoor? <Link to="/register" className="text-champagne-700 font-bold hover:underline ml-1">Create an Account</Link>
                </p>
            </div>
        </div>
    );
